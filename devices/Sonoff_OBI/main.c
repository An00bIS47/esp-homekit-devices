...
uint8_t switch_old_state, switch_state, press_count = 0, device_type_static = 1, relay1_gpio = RELAY1_GPIO;
...
/* ##############################################################################################
    Added #if USE_OBI_SOCKET
*/
#if USE_OBI_SOCKET
    
    if (on){
        gpio_write(RELAY1_GPIO, 0);
        vTaskDelay(50 / portTICK_PERIOD_MS);
        gpio_write(RELAY1_GPIO, 1);
    } else {
        gpio_write(RELAY2_GPIO, 0);
        vTaskDelay(50 / portTICK_PERIOD_MS);
        gpio_write(RELAY2_GPIO, 1);
    }
...
homekit_characteristic_t manufacturer = HOMEKIT_CHARACTERISTIC_(MANUFACTURER, "An00bIS47");
homekit_characteristic_t serial = HOMEKIT_CHARACTERISTIC_(SERIAL_NUMBER, NULL);
homekit_characteristic_t model = HOMEKIT_CHARACTERISTIC_(MODEL, "OBI WLAN KENOBI");
...
void create_accessory_name() {
    ...
    snprintf(name_value, 17, "OBI %02X%02X%02X", macaddr[3], macaddr[4], macaddr[5]);
...
void user_init(void) {
    ...
    wifi_config_init("OBI", NULL, on_wifi_ready);
